...
#define QB2BUS_ENDIANNESS(x)	  ((!!x) ? (1U << 15) : 0U)
...
#define PE_IBUS_BYTES(x) \
	((1U << (x)) - 1U) << (4 - (x)) /* 0x1 = LSB, 0x8 = MSB (BE) */
...
uint32_t pfe_class_cfg_get_text_stat(void *base_va, char_t *buf, uint32_t size,
				     uint8_t verb_level);
void pfe_class_cfg_set_rtable(void *base_va, void *rtable_pa,
