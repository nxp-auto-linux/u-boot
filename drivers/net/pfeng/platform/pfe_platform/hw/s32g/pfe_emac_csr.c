...
static const char *emac_phy_to_str(uint32_t sel)
{
	switch(sel) {
	case 0x00:
		return "G/MII";
...
uint32_t pfe_emac_cfg_get_text_stat(void *base_va, char_t *buf, uint32_t size, uint8_t verb_level)
{
	...
	{
		/*	Get version */
		len += oal_util_snprintf(buf + len, size - len, "Base address              : 0x%llx\n", (uint64_t)base_va);
		...
		reg = (hal_read32((addr_t)base_va + MAC_HW_FEATURE0) >> 28) & 0x07;
		len += oal_util_snprintf(buf + len, size - len, "Active PHY selected       : %s\n", emac_phy_to_str(reg));
	}
	if(verb_level >= 8)
	{
