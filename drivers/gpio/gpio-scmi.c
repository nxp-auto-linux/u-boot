...
static int scmi_gpio_set_open_drain(struct udevice *dev, unsigned int offset,
				    int value)
{
	struct scmi_gpio *gpio_dev = dev_get_priv(dev);
...
static const struct dm_gpio_ops scmi_gpio_ops = {
	...
	.set_open_drain = scmi_gpio_set_open_drain,
...
U_BOOT_DRIVER(scmi_gpio) = {
	...
	.priv_auto_alloc_size = sizeof(struct scmi_gpio),
};
