...
# extract app code + app bl
dd if=u-boot.s32 of=u-boot-tosign.s32 bs=512 skip=1042

# find necessary padding for u-boot
PAD=$((1048576 - $(wc -c < u-boot-tosign.s32)))
dd if=/dev/zero bs=1 count="$PAD" >> u-boot-tosign.s32

# sign extracted app code + app bl after padding
...
skip=1026 seek=1026 count=16 conv=notrunc,fsync

# write hse fw
dd if="$HSE_PATH" of="$DEV_PATH" bs=512 seek=1 conv=notrunc,fsync

# write signature
dd if=u-boot.sign of="$DEV_PATH" bs=512 seek=1025 conv=notrunc,fsync

# write signed u-boot
dd if=u-boot-tosign.s32 of="$DEV_PATH" bs=512 seek=1042 conv=notrunc,fsync
