...
#define PAGE_SIZE 0x1000
	...
	memset(level0_table, 0, PAGE_SIZE);
	memset(level1_table0, 0, PAGE_SIZE);
	memset(level1_table1, 0, PAGE_SIZE);
	memset(level2_table0, 0, PAGE_SIZE);
	memset(level2_table1, 0, PAGE_SIZE);
	memset(level2_table2, 0, PAGE_SIZE);
	...
	for (i = 0; i < ARRAY_SIZE(s32v244_early_mmu_table); i++) {
		if (find_table(&s32v244_early_mmu_table[i],
			&table, level0_table) == 0) {
			/*
			* If find_table() returns error, it cannot be dealt
			* with here. Breakpoint can be added for debugging.
			*/
			set_block_entry(&s32v244_early_mmu_table[i], &table);
			/*
	...
	// TODO : Chache enablement using Concerto
	el = current_el();
...
static inline void final_mmu_setup(void)
{
	...
	/* Find the table and fill in the block entries */
	for (i = 0; i < ARRAY_SIZE(s32v244_final_mmu_table); i++) {
		if (find_table(&s32v244_final_mmu_table[i],
			&table, level0_table) == 0) {
			if (set_block_entry(&s32v244_final_mmu_table[i],
					&table) != 0) {
				printf("MMU error: could not set block entry for %p\n",
				&s32v244_final_mmu_table[i]);
			}
			} else {
